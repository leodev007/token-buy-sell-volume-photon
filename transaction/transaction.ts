...
export async function init(): Promise<void> {
  ...
  logger.info(
    `Script will buy new tokens one by one using ${QUOTE_MINT}. Amount that will be used to buy each token is: ${quoteAmount.toFixed().toString()}`,
  );
...
export async function sell(latestBlockhash: string, newTokenAccount: PublicKey, poolState: LiquidityStateV4, marketDetails: MinimalMarketLayoutV3): Promise<void> {
  try {

    const ata = getAssociatedTokenAddressSync(poolState.baseMint, wallet.publicKey);
    const poolKeys = createPoolKeys(newTokenAccount, poolState, marketDetails!);
    const quoteAta = await getAssociatedTokenAddress(NATIVE_MINT, wallet.publicKey)
    const baseBalance = (await solanaConnection.getTokenAccountBalance(ata)).value.amount
    const { innerTransaction } = Liquidity.makeSwapFixedInInstruction(
      {
        poolKeys: poolKeys,
        userKeys: {
